...
begin
  ...
  if HasEffect('Заморозка') then
    ssprite2d_Draw(IceTex, TX * 32, TY * 32, 32, 32, 0, 150);
...
begin
  ...
  if Enemy <> nil then
    ...
    begin
      ...
      if RHandItem.Count > 0 then
        if RHandItem.Pat.Throw then
          si := @RHandItem;

      if (RHandItem.Count > 0) and (LHandItem.Count > 0) then
        if (RHandItem.Pat.Bow) and (LHandItem.Pat.Arrow) then
          si := @LHandItem;
  ...
  if (WalkTo.X = TX) and (WalkTo.Y = TY) then
    WalkTo.X := -1;
  ...
  begin
    ...
    p := Point2D(TX, TY);
    if WalkTo.X <> -1 then
      Walk(NextStep.X - TX, NextStep.Y - TY);
...
begin
  ...
  x2 := TX + dx;
  y2 := TY + dy;
  if (x2 < 0) or (y2 < 0) or (x2 >= M.Width) or (y2 >= M.Height) then
    Exit;
  ...
  begin
    ...
    if M.Objects.Obj[x2, y2].BlockWalk = True then
      Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      if Cr.HasEffect('Заморозка') then
        Cr.AddEffect('Заморозка', 1);
      ...
      if Random(20) < Cr.GetParamValue('Agility') then
        Dmg := Dmg div 2;
      ...
      begin
        ...
        if Cr = Hero then
          SomeTextOut((ScreenW - 200) div 2, ScreenH div 2 - 30, 'Отравление ядом');
      end;
      Cr.WalkTo.X := -1;
      if Cr = Hero then
        Cr.Enemy := nil;
  ...
  begin
    ...
    begin
      ...
      begin
        if CreateItem(M.Items[i].Pat, M.Items[i].Count) = False then
          Break;
...
begin
  ...
  begin
    ...
    if (tx2 < 0) or (ty2 < 0) or (tx2 >= Map.Width) or (ty2 >= Map.Height) then
      Continue;

    bool := True;
    if M.Objects.Obj[tx2, ty2] <> nil then
      if M.Objects.Obj[tx2, ty2].BlockWalk = True then
        bool := False;
    if M.GetCreature(tx2, ty2) <> nil then
      bool := False;
    if bool = True then
    begin
      Walk(tx2 - TX, ty2 - TY);
...
var
  i: Integer;
begin
  if (Pat.Name = 'GOLEM') and (EffectName = 'Отравление') then
    Exit;
...
begin
  ...
  begin
    ...
    if (Effects[i].Int = 1) and (Effects[i].Name = 'Гипноз') then
      Team := 1;
    if Effects[i].Int < 100 then
      Effects[i].Int := Effects[i].Int - 1;
    if Effects[i].Int = 0 then
      Continue;
    if i <> j then
      Effects[j] := Effects[i];
    j := j + 1;
