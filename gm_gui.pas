...
begin
  ...
  x := ScreenW - 200 + ( 200 - 27 * 3 ) div 2;
  y := 326;
  if PanelN = 0 then asprite2d_Draw( BtnTex, x, y, 32, 32, 0, 2 )
    else asprite2d_Draw( BtnTex, x, y, 32, 32, 0, 1 );
  if PanelN = 0 then i := 1 else i := 0;
  ssprite2d_Draw( BagTex, x + i, y + i, 32, 32, 0, 220 );

  if PanelN = 1 then asprite2d_Draw( BtnTex, x + 27, y, 32, 32, 0, 2 )
    else asprite2d_Draw( BtnTex, x + 27, y, 32, 32, 0, 1 );
  if PanelN = 1 then i := 1 else i := 0;
  ssprite2d_Draw( StatsTex, x + 28 + i, y + i, 32, 32, 0, 220 );

  if PanelN = 2 then asprite2d_Draw( BtnTex, x + 27 * 2, y, 32, 32, 0, 2 )
    else asprite2d_Draw( BtnTex, x + 27 * 2, y, 32, 32, 0, 1 );
  ...
  begin
    ...
    begin
      if DragItem.Pat.Equip = 'RHand' then ssprite2d_Draw( GlowTex, x + 28 - 15, y + 93 - 15, 64, 64, 0 );
      if DragItem.Pat.Equip = 'LHand' then ssprite2d_Draw( GlowTex, x + 106 - 15, y + 93 - 15, 64, 64, 0 );
      if DragItem.Pat.Equip = 'Head' then ssprite2d_Draw( GlowTex, x + 67 - 15, y + 27 - 15, 64, 64, 0 );
      if DragItem.Pat.Equip = 'Body' then ssprite2d_Draw( GlowTex, x + 67 - 15, y + 67 - 15, 64, 64, 0 );
      if DragItem.Pat.Equip = 'Legs' then ssprite2d_Draw( GlowTex, x + 67 - 15, y + 124 - 15, 64, 64, 0 );
    end;
    if Hero.RHandItem.Count > 0 then Item_Draw( @Hero.RHandItem, x + 28, y + 93, Hero.RHandItem.Count, 1 );
    if Hero.LHandItem.Count > 0 then Item_Draw( @Hero.LHandItem, x + 106, y + 93, Hero.LHandItem.Count, 1 );
    if Hero.HeadItem.Count > 0 then Item_Draw( @Hero.HeadItem, x + 67, y + 27, Hero.HeadItem.Count, 1 );
    if Hero.BodyItem.Count > 0 then Item_Draw( @Hero.BodyItem, x + 67, y + 67, Hero.BodyItem.Count, 1 );
    if Hero.LegsItem.Count > 0 then Item_Draw( @Hero.LegsItem, x + 67, y + 124, Hero.LegsItem.Count, 1 );
    ...
    for j := 0 to 4 do
      ...
      begin
        ...
        n := j * 6 + i;
        if n >= Hero.ItemsCnt then Continue;
        if Hero.Items[ n ].Count = 0 then Continue;
  ...
  begin
    ...
    begin
      ...
      if CharPoints > 0 then text_DrawEx( Fnt, x + 120, y + 4 * 20, 1, 0, u_IntToStr( CharPoints ), 255, $FFFF00 );
      ...
      if CharPoints = 0 then text_DrawEx( Fnt, x - 20 + 100 + k, y + CrParamsCnt * 20 + 83 + k, 1, 0, A2U8( 'Принять' ), 255, $FFFFFF, TEXT_HALIGN_CENTER )
        else text_DrawEx( Fnt, x - 20 + 100 + k, y + CrParamsCnt * 20 + 83 + k, 1, 0, A2U8( 'Принять' ), 130, $FFFFFF, TEXT_HALIGN_CENTER );
    end;
    ...
    if NewGame = True then text_Draw( Fnt, x + 51, y + 110, A2U8( 'Навыки' ) )
      else text_Draw( Fnt, x + 38, y + 110, A2U8( 'Навыки' ) );
    if SkillPoints > 0 then text_DrawEx( Fnt, x + 120, y + j, 1, 0, u_IntToStr( SkillPoints ), 255, $FFFF00 );
    ...
    begin
      n := n + 1;
      if StBtnDwn = n then k := 2 else k := 1;
      bool := False;
      if ( NewGame = True ) or ( i > 3 ) then bool := True;
      if ( NewGame = False ) and ( SkillPoints = 0 ) then bool := False;
      if bool = True then
        if Hero.ParamValue[ i ] < 10 then asprite2d_Draw( Btn2Tex, x + 132, y + j - 2, 16, 16, 0, k );
      n := n + 1;      
      if StBtnDwn = n then k := 4 else k := 3;
      if NewGame = True then
    ...
    j := j + 30;
    if Hero.EffectsCnt > 0 then text_Draw( Fnt, x + 35, y + j, A2U8( 'Эффекты' ) );
    for i := 0 to Hero.EffectsCnt - 1 do
    begin
      if i = 3 then Exit;
      j := j + 20;
      text_Draw( Fnt, x, y + j, A2U8( Hero.Effects[ i ].Name ) );
      if Hero.Effects[ i ].Int < 100 then text_Draw( Fnt, x + 120, y + j, u_IntToStr( Hero.Effects[ i ].Int ) );
    end
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        a := 255;
        if k < AllSpells[ i ].Int then a := 120;
    ...
    begin
      ...
      begin
        a := 255;
        if Hero.Mana < Hero.Spells[ i ].Mana then a := 120;
        if ( Hero.Spells[ i ].Name = 'Вампиризм' ) and ( Hero.HasEffect( 'Вампиризм' ) ) then a := 120;
        clr := $FFFFFF;
        if SellSpellN = i then clr := $00FF00;
  ...
  begin
    ...
    if BtnDwn = 1 then asprite2d_Draw( Btn1Tex, ChestX + 149, ChestY + 10, 16, 16, 0, 2 )
      else asprite2d_Draw( Btn1Tex, ChestX + 149, ChestY + 10, 16, 16, 0, 1 );
    if BtnDwn = 2 then asprite2d_Draw( Btn1Tex, ChestX + 149, ChestY + 27, 16, 16, 0, 4 )
      else asprite2d_Draw( Btn1Tex, ChestX + 149, ChestY + 27, 16, 16, 0, 3 );
    ...
    for j := 0 to 1 do
      ...
      begin
        ...
        n := j * 4 + i;
        if n >= LookAtObj.ItemsCnt then Continue;
        if LookAtObj.Items[ n ].Count = 0 then Continue;
        Item_Draw( @LookAtObj.Items[ n ], ChestX + 10 + i * 33, ChestY + 10 + j * 33, LookAtObj.Items[ n ].Count, 1 );
      end;
  end;

  if DragItem <> nil then Item_Draw( DragItem, mouse_X - 16, mouse_Y - 16, DragCount, 2 );
...
begin
  ...
  if STTime > 0 then STTime := STTime - 1;
  ...
  begin
    ...
    begin
      ...
      if StBtnDwn <> 1000 then StBtnDwn := 0;
    end;
    ...
    begin
      ...
      begin
        ...
        if ( NewGame = True ) or ( i > 3 ) then
          if PointInRect( mouse_X, mouse_Y, x + 132, y + j - 2, 16, 16 ) then StBtnDwn := n;
        n := n + 1;
        if NewGame = True then
          if PointInRect( mouse_X, mouse_Y, x + 132 + 14, y + j - 2, 16, 16 ) then StBtnDwn := n;
        j := j + 20;
        if i = 3 then j := j + 50
      end;
      if StBtnDwn <> 0 then
      begin
        ...
        begin
          ...
          begin
            ...
            if n < 4 then CharPoints := CharPoints + 1
              else SkillPoints := SkillPoints + 1;
          end;
        end else
        begin
          n := ( StBtnDwn - 1 ) div 2;
          ...
          begin
            if n < 4 then
              if CharPoints > 0 then CharPoints := CharPoints - 1 else n := -1;
            if n >= 4 then
              if SkillPoints > 0 then SkillPoints := SkillPoints - 1 else n := -1;
            if n <> -1 then Hero.ParamValue[ n ] := Hero.ParamValue[ n ] + 1;
          end;
        end;
      end;
      i := x - 20 + ( 200 - 80 ) div 2;
      ...
      if CharPoints = 0 then
        if PointInRect( mouse_X, mouse_Y, i, j, 80, 26 ) then StBtnDwn := 1000;
    end;
    if NewGame = True then Exit;
  end;
  ...
  begin
    x := ScreenW - 200 + ( 200 - 27 * 3 ) div 2;
    y := 326;
    if PointInRect( mouse_X, mouse_Y, x, y, 3 * 27, 27 ) then
    begin
      i := ( mouse_X - x ) div 27;
      if i < 0 then i := 0;
      if i > 2 then i := 2;
      PanelN := i;
      if PanelN <> 2 then SellSpellN := -1;
    end;
  end;
  ...
  begin
    ...
    if mouse_Click( M_BLEFT ) then
    begin
      if PointInRect( mouse_X, mouse_Y, i + 28, j + 93, 33, 33 ) then Item_UpdateSlot( @Hero.RHandItem, 'RHand' );
      if PointInRect( mouse_X, mouse_Y, i + 106, j + 93, 33, 33 ) then Item_UpdateSlot( @Hero.LHandItem, 'LHand' );
      if PointInRect( mouse_X, mouse_Y, i + 67, j + 27, 33, 33 ) then Item_UpdateSlot( @Hero.HeadItem, 'Head' );
      if PointInRect( mouse_X, mouse_Y, i + 67, j + 67, 33, 33 ) then Item_UpdateSlot( @Hero.BodyItem, 'Body' );
      if PointInRect( mouse_X, mouse_Y, i + 67, j + 124, 33, 33 ) then Item_UpdateSlot( @Hero.LegsItem, 'Legs' );
    end;
    if PointInRect( mouse_X, mouse_Y, i + 28, j + 93, 33, 33 ) then InitItemHint( i + 28, j + 93, @Hero.RHandItem );
    if PointInRect( mouse_X, mouse_Y, i + 106, j + 93, 33, 33 ) then InitItemHint( i + 106, j + 93, @Hero.LHandItem );
    if PointInRect( mouse_X, mouse_Y, i + 67, j + 27, 33, 33 ) then InitItemHint( i + 67, j + 27, @Hero.HeadItem );
    if PointInRect( mouse_X, mouse_Y,  i + 67, j + 67, 33, 33 ) then InitItemHint( i + 67, j + 67, @Hero.BodyItem );
    if PointInRect( mouse_X, mouse_Y, i + 67, j + 124, 33, 33 ) then InitItemHint( i + 67, j + 124, @Hero.LegsItem );
    ...
    if ( mouse_Click( M_BLEFT ) ) and ( imo <> -1 ) then Item_UpdateSlot( @Hero.Items[ imo ], '' );
    if ( mouse_Click( M_BRIGHT ) ) and ( imo <> -1 ) and ( BtnDwn = 0 ) then Item_Use( @Hero.Items[ imo ], Hero );
  ...
  begin
    ...
    begin
      ...
      for i := 0 to Hero.SpellsCnt - 1 do
        ...
        begin
          n := i;
          if Hero.Mana < Hero.Spells[ i ].Mana then n := -1;
          if ( Hero.Spells[ i ].Name = 'Вампиризм' ) and ( Hero.HasEffect( 'Вампиризм' ) ) then n := -1;
          Break;
        end;
      if n <> -1 then
        if SellSpellN = n then SellSpellN := -1
          else SellSpellN := n;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      if PointInRect( mouse_X, mouse_Y, ChestX, ChestY, 4 * 33 + 40, 2 * 33 + 20 ) then BtnDwn := 3;
      if PointInRect( mouse_X, mouse_Y, ChestX + 10, ChestY + 10, 4 * 33, 2 * 33 ) then BtnDwn := 0;
      if PointInRect( mouse_X, mouse_Y, ChestX + 149, ChestY + 10, 16, 16 ) then BtnDwn := 1;
      if PointInRect( mouse_X, mouse_Y, ChestX + 149, ChestY + 27, 16, 16 ) then BtnDwn := 2;
    end;
    if BtnDwn = 3 then
    begin
      ChestX := mouse_X - DragX;
      ChestY := mouse_Y - DragY;
      if ChestX < 0 then ChestX := 0;
      if ChestY < 0 then ChestY := 0;
      if ChestX + 4 * 33 + 40 > ScreenW - 200 then ChestX := ScreenW - 200 - 4 * 33 - 40;
      if ChestY + 2 * 33 + 20 > ScreenH then ChestY := ScreenH - 2 * 33 - 20;
    end;

    if mouse_Up( M_BLEFT ) then
    begin
      if PointInRect( mouse_X, mouse_Y, ChestX + 149, ChestY + 10, 16, 16 ) and ( BtnDwn = 1 ) then LookAtObj := nil;
  ...
  if mouse_Down( M_BLEFT ) = False then BtnDwn := 0;
  ...
  if mouse_Click( M_BRIGHT ) then SellSpellN := -1;

  if HintItem = nil then Hint.Show := False
...
begin
  ...
  if NewGame = True then Exit;
  if BtnDwn <> 0 then Exit;
  if Hero.Health = 0 then Exit;

  if mouse_X > ( ScreenW - 200 ) then Exit;

  if LookAtObj <> nil then
    if PointInRect( mouse_X, mouse_Y, ChestX, ChestY, 4 * 33 + 40, 2 * 33 + 20 ) then Exit;
...
begin
  ...
  if Hero.GetParamValue( 'Интеллект' ) = 0 then Hint.Text := 'Какая-то штука';
  ...
  Hint.Y := y - Hint.H + 5;
  if Hint.X < 0 then Hint.X := 0;
  if Hint.Y < 0 then Hint.Y := 0;
  if Hint.X + Hint.W > ScreenW then Hint.X := ScreenW - Hint.W;
  if Hint.Y + Hint.H > ScreenH then Hint.Y := ScreenH - Hint.H;
  HintItem := Item;
end;
...
begin
  ...
  if STPos.X + w div 2 > ScreenW then STPos.X := ScreenW - w div 2;
